  ...
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT | NodeFilter.SHOW_ELEMENT, {
    acceptNode(node2) {
      if (node2.nodeType === Node.ELEMENT_NODE) {
        if ("result" in (node2.dataset ?? {}) || "inlineResult" in (node2.dataset ?? {}))
          return NodeFilter.FILTER_REJECT;
        if (node2.tagName === "BR")
          return NodeFilter.FILTER_ACCEPT;
        return NodeFilter.FILTER_SKIP;
      }
      if (node2.parentElement?.closest("[data-result], [data-inline-result]"))
        return NodeFilter.FILTER_REJECT;
      return NodeFilter.FILTER_ACCEPT;
    }
  });
...
function appendInterpolatedText(parent, text, fallbackClass = "") {
  ...
  while (match = re.exec(text)) {
    ...
    if (before)
      appendText(parent, before, fallbackClass || null);
  ...
  if (tail2)
    appendText(parent, tail2, fallbackClass || null);
}
...
function renderTokens(source) {
  ...
  try {
    ...
    for (const token of tokens) {
      ...
      if (onDirectiveArgIndex >= 0 && onDirectiveArgIndex < 2 && name === "LITERAL" && typeof token.value === "string") {
...
`);
    ...
    for (const chunk of chunks) {
      if (!chunk.body.length)
        continue;
      const line = Array.isArray(chunk.lines) && chunk.lines.length ? chunk.lines[chunk.lines.length - 1] : 0;
...
class XEditor extends HTMLElement {
  connectedCallback() {
    ...
    this._shadow.appendChild(style2);
...
` : node.textContent;
  ...
  _injectResults() {
    ...
    const makeWidget = ({ result, anchor, pending }) => {
      if (pending) {
        ...
        widget2.dataset.statementId = anchor.id;
    ...
    const previousRenderableTextNode = (fromNode) => {
      ...
      if (node === this._editable) {
        ...
        while (node && node.lastChild)
          node = node.lastChild;
        if (node && node.nodeType === Node.TEXT_NODE && !node.parentElement?.closest("[data-result], [data-inline-result]")) {
          return node;
        }
      }
      while (node && node !== this._editable) {
        if (node.previousSibling) {
          ...
          if (node.nodeType === Node.TEXT_NODE && !node.parentElement?.closest("[data-result], [data-inline-result]")) {
            return node;
          }
    ...
    const walker = document.createTreeWalker(this._editable, NodeFilter.SHOW_ELEMENT, {
      acceptNode: (node) => node.tagName === "BR" ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_SKIP
    });
    let lineIdx = 0;
    let brNode;
  ...
  _onMouseMove(e) {
    const node = e.target instanceof Element ? e.target.closest("[data-token-type], [data-tooltip]") : null;
    if (!node) {
      this._hideTooltip();
      return;
    }
