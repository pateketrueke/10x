var global=globalThis;window.__10x_BUILD_TS='2026-04-12T20:35:40Z';
...
var FAT_ARROW = Symbol("FAT_ARROW");
  ...
  Object.entries(t).forEach(([t2, o2]) => {
    if (t2 !== "key" && t2 !== "open")
      if (t2 === "ref")
        ...
        let l2 = o2 !== true ? o2 : !!c2.includes("-") || c2;
...
function he(e2, t, n) {
  ...
  if (i2 && i2.key && c2.setAttribute("data-key", i2.key), u(n) && (c2 = n(c2, r2, i2, o2) || c2), u(c2))
    return he(c2(), s, n);
  m(i2) || ae(c2, i2, s, n), u(c2.oncreate) && c2.oncreate(c2), u(c2.enter) && c2.enter(), c2.remove = () => Promise.resolve().then(() => u(c2.ondestroy) && c2.ondestroy(c2)).then(() => u(c2.teardown) && c2.teardown()).then(() => u(c2.exit) && c2.exit()).then(() => V(c2)), o2.forEach((e3) => {
    be(c2, e3, s, n);
  });
...
async function ye(e2, t, n, s, r2) {
  ...
  return y(n) ? ge(e2, t, n, s, r2) : (await async function(e3, t2, n2, s2) {
    const r3 = [], i2 = x(t2), o2 = Math.max(e3.childNodes.length, i2.length), c2 = Array.from(e3.childNodes), a2 = new Map, l2 = new Set;
...
function serialize(token, shorten, colorize = (_, x2) => typeof x2 === "undefined" ? literal({ type: _ }) : x2, descriptor = "Root") {
  if (typeof token === "undefined")
    return;
  if (token === null)
...
` || this.isDone() || next === "" || typeof next === "undefined") {
      ...
      case "!":
        ...
        break;
      case "=":
        this.addToken(this.isMatch(">") ? FAT_ARROW : this.isMatch("=") ? EXACT_EQ : EQUAL);
        break;
      case "%":
        this.addToken(MOD);
        break;
...
`)
    ...
    if (isReadable(this.blank) && token === "*" || nextToken === " " && ");:.,".includes(token) && !(token === "." && looksLikeUnitLiteral) || token === " " && (nextToken === "*" || nextToken && isAlphaNumeric(nextToken) && !hasFatArrowAhead(i2))) {
...
class Parser {
  ...
  definitionBlock(token) {
    ...
    if (this.tokens[this.offset] && this.tokens[this.offset].type === FAT_ARROW)
      this.offset++;
    const body = [];
    while (this.offset < this.tokens.length) {
      const cur = this.tokens[this.offset];
      if (!cur || isEOL(cur) || isEOF(cur))
        break;
      if (isText(cur)) {
        this.offset++;
        continue;
    ...
    while (!this.isDone()) {
      ...
      if (isLiteral(token)) {
        ...
        if (curToken && (curToken.type === FAT_ARROW || isLiteral(curToken) && this.peekFatArrow())) {
          push2(Expr.callable(this.definitionBlock(token), tokenInfo));
          continue;
        }
...
var componentInstanceId = 0;
function resetComponentInstanceId(value) {
...
class Eval {
    ...
    const evaluateNode = async (node) => {
      ...
      for (const child of node.children || []) {
        ...
        for (const result of childResults) {
          ...
          if (fixed && typeof fixed === "object" && !Array.isArray(fixed) && typeof fixed.render === "function" && typeof fixed.name === "undefined") {
            let _e2 = this.env;
            while (_e2 && !_e2.__viewCache)
              _e2 = _e2.parent;
            if (_e2 && _e2.__viewCache && _e2.__domRender && typeof document !== "undefined") {
              const idx = _e2.__viewCacheIndex.value++;
              let entry = _e2.__viewCache.get(idx);
              if (!entry) {
                const placeholder = document.createElement("x-slot");
                const dispose = _e2.__domRender(placeholder, fixed);
                entry = { placeholder, dispose };
                _e2.__viewCache.set(idx, entry);
              }
              children.push(entry.placeholder);
            } else {
              const vdom = await fixed.render();
              if (vdom !== null && typeof vdom !== "undefined")
                children.push(vdom);
            }
      ...
      if (isFatArrow) {
        const compFn = Expr.callable({ type: BLOCK, value: { ...compEntry, name: resolvedNode.name } }, this.ctx.tokenInfo);
        const compResult = await this.convert(compFn, [propsArg], "TagComp");
        if (compResult !== undefined)
          return [Expr.value(compResult, this.ctx.tokenInfo)];
        return [];
      }
      return Eval.do([
  ...
  async evalBlocks() {
    ...
    if (this.ctx.isCallable) {
      ...
      for (let i2 = 0;i2 < fixedBody.length; i2++) {
        const stmt = fixedBody[i2];
      }
  ...
  static async map(token, descriptor, environment, parentTokenInfo, state) {
    ...
    const tagToVdom = (node) => {
      ...
      const children = (node.children || []).map((child) => {
        ...
        if (Array.isArray(child) && child.length === 3 && typeof child[0] === "string")
          return child;
    ...
    if (isDirectiveStmt(value.signal)) {
      ...
      if (!_ownerEnv.__signalCache)
        _ownerEnv.__signalCache = new Map;
      ...
      if (isDone2) {} else {
        ...
        let componentInstanceId2 = environment.__componentInstanceId;
        let envPtr = environment.parent;
        while (envPtr && (!componentName || !componentInstanceId2)) {
          if (envPtr.__componentName)
            componentName = envPtr.__componentName;
          if (envPtr.__componentInstanceId)
            componentInstanceId2 = envPtr.__componentInstanceId;
          envPtr = envPtr.parent;
        }
        const namespacedName = componentName && componentInstanceId2 ? `${componentName}$${componentInstanceId2}.${signalName}` : signalName;
        const moduleUrl = componentName && componentInstanceId2 ? `${componentName}$${componentInstanceId2}` : undefined;
    ...
    if (!isDirectiveStmt(value.render) && isDirectiveStmt(value.html)) {
      ...
      let domRender = null;
      try {
        domRender = resolveRuntimeFn("render");
      } catch (_) {}
      ...
      const view = html2(async () => {
        ...
        for (let i2 = 0;i2 < localNames.length; i2++) {
          ...
          if (!(head2 && head2.isObject && head2.value && head2.value.signal))
            continue;
          const plain = head2.value;
          if (!isSignalValue(plain))
            continue;
          signalMap2.set(name, plain);
    ...
    if (isDirectiveStmt(value.on)) {
      ...
      const _callable = _rawOnBody.length === 1 && _rawOnBody[0] && _rawOnBody[0].isCallable ? _rawOnBody[0] : args.length === 1 && args[0] && args[0].isCallable ? args[0] : null;
      ...
      if (_signalName && _bodyTokens) {
        ...
        const handlerFn = async () => {
          const findSignalEntry = async (name, env) => {
            let e2 = env;
            while (e2) {
              const entry = e2.locals && e2.locals[name];
              if (entry) {
                const [head2] = entry.body || [];
                const v2 = head2 && typeof head2.valueOf === "function" ? head2.valueOf() : null;
                if (v2 && typeof v2.set === "function" && typeof v2.peek === "function") {
                  return v2;
                }
              }
              e2 = e2.parent;
          ...
          if (signalObj && typeof signalObj.set === "function" && typeof signalObj.peek === "function") {
            const current2 = signalObj.peek();
            const innerEnv = new Env(environment);
            innerEnv.def(signalName, Expr.value(current2, parentTokenInfo));
            const [result] = await Eval.do(bodyTokens, innerEnv, "OnBody", true, parentTokenInfo);
            const next = result ? toPlain(result) : current2;
            signalObj.set(next);
          }
        };
  ...
  return {
    ...
    setup({ Env: Env2, Expr: Expr2 }) {
      Env2.register = (num, kind) => {
        ...
        for (let i2 = 0;i2 < safeUnits.length; i2++) {
          const result = safeUnits[i2](num, kind);
          if (result)
            return result;
        }
      ...
      Env2.shared.Runtime = exports_runtime;
    }
  };
}

// src/editor/editor-runtime-shared.js
var SYMBOL_NAME2 = (sym) => {
  if (!sym || typeof sym !== "symbol")
    return "";
  return sym.toString().match(/Symbol\((.+)\)/)?.[1] ?? "";
};
var symbolCatalogReady = false;
function ensureSymbolCatalog() {
  if (symbolCatalogReady)
    return;
  symbolCatalogReady = true;
  ensureDefaultMappings();
}
function catalogSymbolHint(symbolText) {
...
function isUnitToken(token) {
  return SYMBOL_NAME2(token?.type) === "NUMBER" && token?.value && typeof token.value.toString === "function" && /[A-Za-z]/.test(String(token.value.toString()));
}
function formatRuntimeValue(value, max = 180) {
...
function assignedNameFromSource(source) {
  const text = String(source || "");
  const match = text.match(/^\s*([A-Za-z_][A-Za-z0-9_!?-]*)\s*=/);
  return match ? match[1] : "";
}
function annotationTypeForSource(source, env) {
  const name = assignedNameFromSource(source);
  if (!name || !env || typeof env.getAnnotation !== "function")
    return "";
  const ann = env.getAnnotation(name);
...
function prettyTypeName(typeName) {
  const canon = canonicalTypeName2(typeName);
  if (canon === "number")
    return "num";
  if (canon === "string")
    return "str";
...
function annotationWarningForSource(source, env, runtimeType) {
  ...
  if (!inferred || inferred === "unknown")
    return null;
  const expectedCanon = canonicalTypeName2(expected);
  const inferredCanon = canonicalTypeName2(inferred);
  if (!expectedCanon || !inferredCanon || expectedCanon === inferredCanon)
    return null;
  return {
...
`;
function emitEditorRuntimeLog(level, ...args) {
...
async function bootstrapEnv(env, executeFn) {
  if (!env || env.__xEditorBootstrapDone)
    return;
...
// src/editor/editor-contenteditable.js
...
function collectSignalSnapshot() {
  const snapshot = [];
  for (const [name, signal2] of getSignalRegistry().entries()) {
    const history = Array.isArray(signal2?._history) ? signal2._history.slice(-20) : [];
    snapshot.push({
      id: signal2?._devtoolsId || null,
      name: String(name),
      moduleUrl: signal2?._moduleUrl || "global",
      value: toSerializable(read(signal2)),
      subs: signal2?.subs ? signal2.subs.size : 0,
...
function tokenClass(token) {
  const n = SYMBOL_NAME2(token.type);
  switch (n) {
    case "COMMENT":
    case "COMMENT_MULTI":
      return "xt-comment";
...
function tokenTooltipLabel(token) {
  ...
  if (n === "TEXT" && token?.value && typeof token.value === "object") {
    const kind = SYMBOL_NAME2(token.value.kind);
    if (kind === "BLOCKQUOTE")
      return "Block quote";
    if (kind === "OL_ITEM")
  ...
  switch (n) {
    case "NUMBER":
      if (token?.value && typeof token.value === "object" && typeof token.value.kind === "string" && token.value.kind.trim()) {
    ...
    case "SYMBOL": {
      ...
      if (hint)
        return `Symbol (${token?.value}) • ${hint}`;
      return "Symbol";
    }
    case "LITERAL":
      return "Literal";
    ...
    case "COMMENT_MULTI":
      return "Comment";
    case "BOLD":
      return "Bold markdown";
    case "ITALIC":
      return "Italic markdown";
    case "CODE":
...
function tokenText(token) {
  ...
  const v2 = token.value;
  if (typeof v2 === "string")
    return v2;
  if (typeof v2 === "number")
    return String(v2);
  if (n === "NUMBER" && v2 && typeof v2 === "object") {
    if (typeof v2.num === "number" && typeof v2.kind === "string") {
      return `${v2.num}${v2.kind}`;
    }
    if (typeof v2.toString === "function" && v2.toString !== Object.prototype.toString) {
      return String(v2);
    }
  }
  if ((n === "LITERAL" || n === "SYMBOL" || n === "DIRECTIVE") && v2 && typeof v2 === "object") {
    if (typeof v2.value === "string" || typeof v2.value === "number")
      return String(v2.value);
    if (typeof v2.name === "string")
      return v2.name;
    if (typeof v2.kind === "string")
      return v2.kind;
    if (typeof v2.toString === "function" && v2.toString !== Object.prototype.toString) {
      return String(v2);
    }
...
function interpolatedStringBody(token) {
  ...
  for (let i2 = 0;i2 < parts.length; i2++) {
    const part = parts[i2];
    const kind = SYMBOL_NAME2(part?.type);
    if (kind === "PLUS")
      continue;
    if (kind === "OPEN" && part?.value === "#{") {
      ...
      while (i2 < parts.length) {
        const inner = parts[i2];
        const innerKind = SYMBOL_NAME2(inner?.type);
...
function hasTextBody(buffer) {
  return buffer.some((chunk) => typeof chunk === "string" ? chunk.length > 0 : !!chunk);
}
function resolveStructuralPrefix(textTokenValue) {
  ...
  const kind = SYMBOL_NAME2(textTokenValue.kind);
  if (kind === "BLOCKQUOTE") {
    return {
      prefix: hasBody ? "> " : ">",
      cls: "xt-prose"
  ...
  const walk = (node) => {
    if (node === targetNode) {
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        for (let i2 = 0;i2 < Math.min(targetOffset, node.childNodes.length); i2++) {
          offset += visibleTextLen(node.childNodes[i2]);
        }
      }
      return true;
    }
  ...
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT | NodeFilter.SHOW_ELEMENT, {
    acceptNode(node2) {
      if (node2.nodeType === Node.ELEMENT_NODE) {
        if ("result" in (node2.dataset ?? {}))
          return NodeFilter.FILTER_REJECT;
        if (node2.tagName === "BR")
          return NodeFilter.FILTER_ACCEPT;
        return NodeFilter.FILTER_SKIP;
      }
      if (node2.parentElement?.closest("[data-result]"))
        return NodeFilter.FILTER_REJECT;
      return NodeFilter.FILTER_ACCEPT;
    }
  });
  ...
  while (node = walker.nextNode()) {
    if (node.tagName === "BR") {
      if (remaining === 0) {
        ...
        const sel2 = getSelection(shadowRoot);
    ...
    if (parts[i2]) {
      if (cls || tooltip) {
        ...
        if (cls)
          span.className = cls;
        if (tooltip)
          span.dataset.tokenType = tooltip;
        span.textContent = parts[i2];
        parent.appendChild(span);
      } else {
        parent.appendChild(document.createTextNode(parts[i2]));
      }
    }
...
function appendHtmlTagToken(parent, tag, baseClass = "") {
  ...
  while (match = attrRegex.exec(attrs)) {
    ...
    if (typeof match[3] === "string") {
      ...
      if (between)
        appendPart(between, "xt-tag");
      const rawValue = match[3];
      const isQuoted = rawValue.length >= 2 && (rawValue.startsWith('"') && rawValue.endsWith('"') || rawValue.startsWith("'") && rawValue.endsWith("'"));
      if (isQuoted) {
        const quote2 = rawValue[0];
        const inner = rawValue.slice(1, -1);
        appendPart(quote2, "xt-tag");
        appendInterpolatedText(parent, inner, [baseClass, "xt-attr-value"].filter(Boolean).join(" "), "HTML attribute value");
        appendPart(quote2, "xt-tag");
      } else {
        appendInterpolatedText(parent, rawValue, [baseClass, "xt-attr-value"].filter(Boolean).join(" "), "HTML attribute value");
      }
    }
...
function appendHtmlTaggedText(parent, text, baseClass = "", fallbackTooltip = "") {
  ...
  while (match = tagRegex.exec(text)) {
    ...
    if (before)
      appendInterpolatedText(parent, before, baseClass || null, fallbackTooltip);
    appendHtmlTagToken(parent, match[0], baseClass);
    cursor = match.index + match[0].length;
  }
  const tail2 = text.slice(cursor);
  if (tail2)
    appendInterpolatedText(parent, tail2, baseClass || null, fallbackTooltip);
}
function appendInterpolatedText(parent, text, fallbackClass = "", fallbackTooltip = "") {
  ...
  while (match = re2.exec(text)) {
    const before = text.slice(cursor, match.index);
    if (before)
      appendText(parent, before, fallbackClass || null, fallbackTooltip);
...
${expr}`, "raw").filter((tok, idx) => {
          const kind = SYMBOL_NAME2(tok?.type);
          if (kind === "EOF")
            return false;
          if (idx === 0 && kind === "EOL")
            return false;
...
`) {
  ...
  const tail2 = text.slice(cursor);
  if (tail2)
    appendText(parent, tail2, fallbackClass || null, fallbackTooltip);
}
function appendToken(parent, token) {
  ...
  if (n === "EOL") {
    ...
    if (typeof token.line === "number")
      eol.dataset.line = String(token.line);
    eol.textContent = ".";
    parent.appendChild(eol);
    return;
  }
  if (n === "STRING" && Array.isArray(token.value)) {
    const text2 = interpolatedStringBody(token);
    if (token?.kind === "markup" || /<\/?[A-Za-z]/.test(text2)) {
      appendHtmlTaggedText(parent, text2, "", "");
      return;
    }
    const cls = tokenClass(token);
    appendInterpolatedText(parent, quoteStringDisplay(text2), cls, tooltip);
    return;
  }
  ...
  if (Array.isArray(token.value)) {
    ...
    if (cls) {
      ...
      if (tooltip)
        wrapper.dataset.tokenType = tooltip;
    }
  ...
  if (token.value && typeof token.value === "object" && Array.isArray(token.value.buffer)) {
    ...
    if (prefix)
      appendText(wrapper, prefix, null);
...
`);
  for (let i2 = 0;i2 < lines.length; i2++) {
    if (lines[i2])
      parent.appendChild(document.createTextNode(lines[i2]));
...
function getFunctionDefinitionMeta(source) {
  ...
  const args = argsText ? argsText.split(",").map((part) => part.trim()).filter(Boolean) : [];
  return {
    name,
    args,
...
function buildStatementAnchors(source) {
  ...
  try {
    ...
    const chunkInfo = chunks.map((c2, i2) => {
      const first = c2.body?.[0];
      const last = c2.body?.[c2.body.length - 1];
      const types = c2.body?.map((t) => String(t?.type).replace("Symbol(", "").replace(")", "")) || [];
      return {
        idx: i2,
        bodyLen: c2.body?.length || 0,
...
`);
    ...
    const isProseOnlyChunk = (chunk) => {
      ...
      return chunk.body.every((tok) => {
        if (!tok)
          return false;
        const typeName = SYMBOL_NAME2(tok.type);
    ...
    for (const chunk of chunks) {
      ...
      if (!chunk.body.length) {
        console.log("[buildAnchors] skip empty body");
        continue;
      }
      const eolToken = chunk.body[chunk.body.length - 1];
      let line = typeof eolToken?.tokenInfo?.line === "number" ? eolToken.tokenInfo.line : Array.isArray(chunk.lines) && chunk.lines.length ? chunk.lines[chunk.lines.length - 1] : 0;
      const firstLine = Array.isArray(chunk.lines) && chunk.lines.length ? chunk.lines[0] : line;
...
${sourceLines[end]}`;
      ...
      if (!statementSource.trim()) {
        console.log("[buildAnchors] skip empty trimmed");
        continue;
      }
...
class XEditor extends HTMLElement {
  connectedCallback() {
    ...
    this._shadow.appendChild(style3);
    this._shadow.appendChild(this._editable);
    this._shadow.appendChild(this._tooltip);
    this._editable.addEventListener("beforeinput", (e2) => this._onBeforeInput(e2));
    this._editable.addEventListener("input", () => this._onInput());
    this._editable.addEventListener("keydown", (e2) => this._onKeydown(e2));
    this._editable.addEventListener("mousedown", (e2) => this._onMouseDown(e2));
    this._editable.addEventListener("click", (e2) => this._onClick(e2));
    this._editable.addEventListener("mousemove", (e2) => this._onMouseMove(e2));
  ...
  _extractText() {
    const walker = document.createTreeWalker(this._editable, NodeFilter.SHOW_TEXT | NodeFilter.SHOW_ELEMENT, {
      acceptNode(node2) {
        if (node2.nodeType === Node.ELEMENT_NODE) {
          if ("result" in (node2.dataset ?? {})) {
            return NodeFilter.FILTER_REJECT;
...
` : node.textContent;
  ...
  _applySource(source, cursorOffset = null) {
    ...
    console.log("[editor] anchors:", this._anchors.map((a2) => ({ id: a2.id, prose: a2.isProseOnly, src: a2.source.substring(0, 40) })));
  ...
  _makeResultWidget({
    ...
    if (pending) {
      ...
      widget2.dataset.result = "";
      if (statementId)
        widget2.dataset.statementId = statementId;
      if (inlineId)
        widget2.dataset.inlineId = inlineId;
      widget2.setAttribute("contenteditable", "false");
      widget2.classList.add("loading");
      const label2 = document.createElement("span");
      label2.textContent = "…";
      widget2.appendChild(label2);
    ...
    if (statementId)
      widget.dataset.statementId = statementId;
    if (inlineId)
      widget.dataset.inlineId = inlineId;
  ...
  _injectResults() {
    ...
    if (!this._anchors.length && !this._inlineAnchors.length)
      return;
    ...
    this._anchors.forEach((anchor) => {
      ...
      if (!pending && !result && !fallbackUnitText)
        return;
      if (!byLine.has(anchor.line))
    ...
    this._editable.querySelectorAll("[data-eol]").forEach((node) => {
      ...
      if (!Number.isInteger(line))
        return;
      if (!eolByLine.has(line))
        eolByLine.set(line, []);
      eolByLine.get(line).push(node);
    });
    const sortedLines = Array.from(byLine.keys()).sort((a2, b2) => a2 - b2);
    for (const line of sortedLines) {
      const lineResults = byLine.get(line) || [];
      const eols = eolByLine.get(line) || [];
      for (let i2 = 0;i2 < lineResults.length; i2++) {
        const entry = lineResults[i2];
        const eol = eols[Math.min(i2, Math.max(0, eols.length - 1))];
        if (!eol)
          continue;
        if (entry.pending)
          eol.dataset.eolPending = "";
        else if (entry.result?.errorText)
          eol.dataset.eolError = "";
...
`);
  ...
  _bindInlineAnchorNodes() {
    this._inlineAnchorNodes = new Map;
    if (!this._inlineAnchors.length)
      return;
    const openDelims = Array.from(this._editable.querySelectorAll(".xt-interp-delim")).filter((node) => node.textContent === "#{");
    for (let i2 = 0;i2 < this._inlineAnchors.length && i2 < openDelims.length; i2++) {
      const inlineId = this._inlineAnchors[i2].id;
      const node = openDelims[i2];
  ...
  _startWorker() {
    try {
      ...
      const inheritedRev = moduleUrl.searchParams.get("v") || moduleUrl.searchParams.get("debug") || moduleUrl.searchParams.get("t") || "";
      const rev2 = globalThis.__X_EDITOR_WORKER_REV || inheritedRev || "";
      if (rev2)
        workerUrl.searchParams.set("v", String(rev2));
      ...
      this._worker.addEventListener("message", ({ data }) => {
        ...
        if (!requestId || requestId < this._appliedEvalRequestId)
          return;
  ...
  _requestEval({ preserveKilled = false } = {}) {
    ...
    const needsMainThreadRuntime = activeStatements.some(({ source }) => {
      ...
      return /\b(render|renderShadow|on|style)\s*\(/.test(text2) || /@(?:render|on|style)\b/.test(text2);
    });
      ...
      for (const statement of statements) {
        ...
        try {
          ...
          if (isFunctionDefinitionSource(statementSource)) {
            ...
            const runtimeType = inferRuntimeType2(result);
          ...
          if (unitDisplay && (!this._resultsById.has(statement.statementId) || !this._resultsById.get(statement.statementId)?.resultText?.trim())) {
            this._resultsById.set(statement.statementId, {
              statementId: statement.statementId,
  ...
  _onKeydown(e2) {
    ...
    if (meta && e2.key === "z" && !e2.shiftKey) {
      e2.preventDefault();
      this._undo();
      return;
    }
    if (meta && (e2.key === "y" || e2.key === "z" && e2.shiftKey)) {
      e2.preventDefault();
      this._redo();
      return;
    }
    if (meta && e2.key === "Enter") {
      e2.preventDefault();
    ...
    if (this._isReplMode() && !meta && !e2.shiftKey && e2.key === "ArrowUp" && this._browseReplHistory(-1)) {
      e2.preventDefault();
      return;
    }
    if (this._isReplMode() && !meta && !e2.shiftKey && e2.key === "ArrowDown" && this._browseReplHistory(1)) {
      e2.preventDefault();
      return;
    }
    if (e2.key === "Tab") {
  ...
  _onMouseDown(e2) {
    const copyBtn = e2.target instanceof Element ? e2.target.closest("[data-copy-btn]") : null;
    const cancelBtn = e2.target instanceof Element ? e2.target.closest("[data-cancel-btn]") : null;
    if (!copyBtn && !cancelBtn)
      return;
    e2.preventDefault();
  }
  ...
  _setCopyFeedback(resultNode, copyBtn, ok) {
    ...
    this._copyFeedbackTimer = setTimeout(() => {
      if (!resultNode.isConnected || !copyBtn.isConnected)
        return;
  ...
  async _onClick(e2) {
    ...
    if (cancelBtn) {
      e2.preventDefault();
      e2.stopPropagation();
      const resultNode2 = cancelBtn.closest("[data-result]");
      const statementId = resultNode2?.dataset.statementId;
      if (statementId) {
  ...
  _positionTooltip(x2, y2) {
    ...
    let left = x2 + 12;
    let top = y2 + 14;
    if (left + tipW > hostRect.width - pad)
      left = Math.max(pad, hostRect.width - tipW - pad);
    if (top + tipH > hostRect.height - pad)
      top = Math.max(pad, y2 - tipH - 10);
    this._tooltip.style.left = `${left}px`;
    this._tooltip.style.top = `${top}px`;
  }
  _showTooltip(label, x2, y2, target) {
    if (!this._tooltip || !label)
      return;
    this._tooltip.textContent = label;
    this._tooltip.style.display = "block";
    this._tooltipTarget = target;
    this._positionTooltip(x2, y2);
  }
  _onMouseMove(e2) {
    const node = e2.target instanceof Element ? e2.target.closest("[data-token-type], [data-tooltip]") : null;
    if (!node) {
      this._hideTooltip();
      return;
    }
    const hostRect = this.getBoundingClientRect();
    const x2 = e2.clientX - hostRect.left;
    const y2 = e2.clientY - hostRect.top;
    ...
    this._tooltipTimer = setTimeout(() => {
      this._showTooltip(label, x2, y2, node);
    }, 150);
  }
}
customElements.define("x-editor", XEditor);
console.log("[editor] defined x-editor");
var editor_contenteditable_default = XEditor;
export {
  createBrowserAdapter,
  editor_contenteditable_default as TenxEditorCE
};
