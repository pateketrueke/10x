...
`;
  ...
  static sub(body, params) {
    if (Array.isArray(body)) {
      ...
      for (let i2 = 0, c2 = self2.length;i2 < c2; i2++) {
        if (self2[i2].isCallable && params[self2[i2].value.name]) {
          const fixedBody = params[self2[i2].value.name].slice();
          const fixedName = fixedBody.pop();
          self2[i2].value.name = fixedName.value;
          self2[i2].value.body = Expr.sub(self2[i2].value.body, params);
          self2.splice(i2 - 1, 0, ...fixedBody);
        } else if (isLiteral(self2[i2]) && typeof self2[i2].value === "string") {
          if (!params[self2[i2].value])
            continue;
          c2 += params[self2[i2].value].length - 1;
          self2.splice(i2, 1, ...params[self2[i2].value]);
        } else {
          self2[i2] = Expr.sub(self2[i2], params);
        }
      }
      return self2;
    }
  ...
  static wrap(self2) {
    return async (fn, args, label) => {
      ...
      try {
        ...
        const [value] = await Eval.do(body, scope, label, false, self2.ctx.tokenInfo);
        return value ? Expr.plain(value, self2.convert, `<${fn.name || "Function"}>`) : undefined;
      } catch (e2) {
        raise(e2.message.replace(/\sat line.*$/, ""), self2.ctx.tokenInfo);
      }
    };
  }
