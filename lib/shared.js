...
import Time from '../src/lib/std/time.js';
import Num from '../src/lib/std/num.js';
import UUID from '../src/lib/std/uuid.js';
import Http from '../src/lib/std/http.js';
...
export default ({ Env }, argv = process.argv.slice(2)) => {
  const _argv = argv;
  ...
  Env.shared = {
    Proc: {
      ...
      setenv: (k, v) => { process.env[k] = String(v); },
